...
<details>
<summary><strong>36개 전체 탐지 시나리오 리스트 보기 (클릭)</strong></summary>
<br>
...
- [x] **Brute Force 의심**: 짧은 시간 내 잦은 로그인 실패 후 성공하는 패턴 감지
- [x] **MFA 무력화 시도**: MFA(다요소 인증) 디바이스 비활성화, 삭제, 우회 재등록 시도 탐지
- [x] **세션 하이재킹 의심**: 짧은 시간 내 **잦은 콘솔 세션 교체** 및 비정상적 세션 활동 감지
- [x] **계정 복구 시도**: 잠긴 계정 복구 시도 및 비밀번호 재설정 행위 탐지
...
- [x] **버킷 ACL 변경**: 버킷 자체의 Access Control List를 수정하여 퍼블릭으로 전환하는 시도
